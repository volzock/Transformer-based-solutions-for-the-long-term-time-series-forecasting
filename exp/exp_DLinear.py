        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if (i + 1) % 500 == 0:
                    ...
                    print("-"*80)
                    print(f"    Loss : {loss.item():2.7f} (MSE)")
                    
                    speed = (time.time() - time_now) / iter_count
                    left_time = speed * ((self.args.train_epochs - epoch) * train_steps - i)
                    print(f'    Speed: {speed:.4f} sec/iter \n    Left time: {left_time:.4f} sec')
                    print("-"*80)
                    iter_count = 0
                    time_now = time.time()
                ...
                if self.args.use_amp:
                    ...
                    loss.backward()
                    model_optim.step()
            print(f'{" "*35}Epoch {epoch + 1} Summery {" "*25}')
            print("-"*80)
            print("     Cost time: {}".format(epoch + 1, time.time() - epoch_time))
            train_loss = np.average(train_loss)
            if not self.args.train_only:
                vali_loss = self.vali(vali_data, vali_loader, criterion)
                test_loss = self.vali(test_data, test_loader, criterion)

                print(f"     Steps: {train_steps}, Train Loss: {train_loss:.7f} \t Valid Loss: {vali_loss:.7f} \t Test Loss: {test_loss:.7f}")
                early_stopping(vali_loss, self.model, path)
            else:
                print(f"     Steps: {train_steps} \t Train Loss: {train_loss:.7f}")
                early_stopping(train_loss, self.model, path)

            if early_stopping.early_stop:
                print(f'{" "*22}Early stopping {" "*30}')
                break
            print("="*80)
