...
def Visual(true, preds=None, name='./pic/test.pdf'):
    ...
    fig, ax = plt.subplots()
    sns.lineplot(data=true, label='GroundTruth', color= blind_colors[-3], linewidth=2, ax=ax)
    if preds is not None:
        sns.lineplot(data=preds, label='Prediction', color=RdBu_colors[0], linewidth=2, ax=ax)
    plt.legend()
